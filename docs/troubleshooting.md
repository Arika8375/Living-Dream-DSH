...
```powershell
# 1. 检查 MCP 服务器是否能独立运行
python G:\qwenwork_files\memory-mcp\server.py
...
# 3. 检查代理日志
Get-Content "D:\DeepSeek work space\dsh-phone\proxy.log" -Tail 50
```
...
# 2. 检查视觉配置
Get-Content "G:\qwenwork_files\dsh_vision_config.json"

# 3. 测试 API 连通性
python G:\qwenwork_files\test_vision_zhipu.py
```
